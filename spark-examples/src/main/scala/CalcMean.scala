...
object CalcMean {
  def main(args: Array[String]): Unit = {
    ...
    val result = words.combineByKey(
      (v: Int) => (v, 1), // create combiner，创建key的累加器的初始值，每个分区key首次出现时调用
      (acc: (Int, Int), v: Int) => (acc._1 + v, acc._2 + 1), // merge value，分区中key已经存在时，将新的值累加到累加器中
      (acc1: (Int, Int), acc2: (Int, Int)) => (acc1._1 + acc2._1, acc1._2 + acc2._2), // merge combiners，将两个分区的同一个key的累加器进行累加
      new HashPartitioner(2) // 指定分区方式
